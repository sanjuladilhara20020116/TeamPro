...
const buildLocalSummary = (reports) => {
  ...
  const blockerSummary = openBlockers.length
    ? openBlockers
        .map((item) => `- ${item.member} on ${item.project}: ${item.blocker}`)
        .join("\n")
  ...
  try {
    ...
    if (
      ...
      return res.status(200).json({
        ...
        answer:
          "OpenRouter API key is not configured, so I generated this summary locally.\n\n" +
          buildLocalSummary(reports),
      });
    }
...
- Do not guess.
- Keep answers professional, short, and clear.
- Do not expose passwords, tokens, API keys, or private system details.
- If the question is unrelated to reports, redirect the user to report-related help.
`;
...
`;
    ...
    const openRouterResponse = await fetch(
      "https://openrouter.ai/api/v1/chat/completions",
      {
        ...
        headers: {
          ...
          // Optional headers for OpenRouter app identification
          "HTTP-Referer": process.env.APP_URL || "http://localhost:5173",
          "X-Title": process.env.APP_NAME || "TeamPro",
        },
        body: JSON.stringify({
    ...
    if (!openRouterResponse.ok) {
      return res.status(200).json({
        ...
        source: "local-summary",
        answer:
          "OpenRouter API failed, so I generated this summary locally.\n\n" +
          "Error: " +
    ...
    const answer =
      ...
      "OpenRouter response received, but no text was returned.";
